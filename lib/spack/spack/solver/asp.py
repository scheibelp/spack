...
class ConstraintOrigin(enum.Enum):
    """Generates identifiers that can be passed into the solver attached to constraints,
    and then later retrieved to determine the origin of those constraints when ``SpecBuilder``
    creates Specs from the solve result.
    """
    ...
    @staticmethod
    def strip_suffix(source: str) -> Tuple[int, Optional[str]]:
        """Take a combined package/kind ID generated by ``append_suffix``, and extract the
        package ID and an associated weight.
        """
        if not source:
            return -1, None
        ...
        for spec in self._specs.values():
            ...
            for flag_type in spec.compiler_flags.valid_compiler_flags():
                ...
                def _order_index(flag_group):
                    ...
                    type_index, pkg_source = ConstraintOrigin.strip_suffix(source)
